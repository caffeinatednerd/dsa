...
public class Main {
  ...
  public static BSTPair checkBST(Node node) {
      ...
      curPair.max = Math.max(node.data, Math.max(lpair.max, rpair.max));
      curPair.min = Math.min(node.data, Math.min(lpair.min, rpair.min));
      
      if(lpair.isBST && rpair.isBST && node.data >= lpair.max && node.data <= rpair.min) {
